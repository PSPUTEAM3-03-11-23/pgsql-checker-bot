...
import (
	...
	"hackathon-tg-bot/internal/app/service/geometry"
	"hackathon-tg-bot/internal/app/validator"
	"hackathon-tg-bot/internal/app/validator/AreaPointValidator"
...
func ValidateArea(area *entity.Area) *errorHandler.HttpErr {
	...
	if area.AreaPoints == nil {
		return errorHandler.New("areaPoints is empty", http.StatusBadRequest)
	}

	if len(area.AreaPoints) < 3 {
		return errorHandler.New("size of areaPoints must be greater than 2", http.StatusBadRequest)
	}
	...
	if geometryService.IsAllPointsOnOneLine(&area.AreaPoints) {
		return errorHandler.New("all points are on the same line", http.StatusBadRequest)
	}

	for i := 0; i < len(area.AreaPoints); i++ {
		for j := i + 1; j < len(area.AreaPoints); j++ {
			if area.AreaPoints[i].IsEqual(&area.AreaPoints[j]) {
				return errorHandler.New("area contains duplicate points", http.StatusBadRequest)
			}
		}
	}
	...
	for i := 0; i < len(lineSegments); i++ {
		for j := i + 1; j < len(lineSegments); j++ {
			if geometryService.IsIntersects(&lineSegments[i], &lineSegments[j]) {
				return errorHandler.New("area must be non self-intersecting", http.StatusBadRequest)
			}
		}
	}
...
func ValidateIntersectionAndAreaRepeats(area *entity.Area, existingArea *entity.Area) *errorHandler.HttpErr {
	...
	if area.Name == existingArea.Name {
		return errorHandler.New("area with this name already exists", http.StatusConflict)
	}
	...
	if len(area.AreaPoints) == len(existingArea.AreaPoints) {
		...
		if isFullOverlapFound {
			return errorHandler.New(fmt.Sprintf("area with these points already exists with id %d", existingArea.Id), http.StatusConflict)
		}
	...
	for i := 0; i < len(existingLineSegments); i++ {
		for j := 0; j < len(areaLineSegments); j++ {
			if geometryService.IsIntersects(&existingLineSegments[i], &areaLineSegments[j]) {
				return errorHandler.New(fmt.Sprintf("area intersects with area with id %d", existingArea.Id), http.StatusBadRequest)
			}
		}
	}
	...
	for _, point := range area.AreaPoints {
		if geometryService.IsPointInsideArea(&point, existingArea, false) {
			return errorHandler.New(fmt.Sprintf("area lays inside area with id %d", existingArea.Id), http.StatusBadRequest)
		}
	}

	for _, point := range existingArea.AreaPoints {
		if geometryService.IsPointInsideArea(&point, area, false) {
			return errorHandler.New(fmt.Sprintf("area contains area with id %d", existingArea.Id), http.StatusBadRequest)
		}
	}
	return nil
}
